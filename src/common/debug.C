...
// have to include this at the end of errlog.C.
...
__debug::__debug(char *n, char *f) : ErrLog(n, log_to_unix_file, f?f:"-")
{
	...
	if(strstr(_flags,"all")) {
		mask |= _all;
	} else if(!none()) {
		char *s;
		if((s=re_comp(_flags)) != 0) {
