...
/*
 *  $Header: /p/shore/shore_cvs/src/common/vec_t.c,v 1.46 1996/05/03 03:12:17 kupsch Exp $
 */
...
void cvec_t::split(size_t l1, cvec_t& v1, cvec_t& v2)
{
    size_t min = 0;
    int i;
    for (i = 0; i < _cnt; i++)  {
...
int cvec_t::cmp(const cvec_t& v, size_t* common_size) const
{
	...
	if ((j2 += min) >= _base[i2].len)	{ j2 = 0; ++i2; }
