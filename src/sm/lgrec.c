...
/*
 *  $Id: lgrec.c,v 1.49 1996/05/06 20:10:53 nhall Exp $
 */
#define SM_SOURCE
#define LGREC_C
#ifdef __GNUG__
...
shpid_t lg_tag_indirect_h::_pid(uint4 pid_num) const
{
    ...
    uint4 idx = pid_num/lgindex_p::max_pids;
...
rc_t
lgdata_p::format(const lpid_t& pid, tag_t tag, uint4_t flags)
{
...
rc_t
lgindex_p::format(const lpid_t& pid, tag_t tag, uint4_t flags)
{
    W_DO( page_p::format(pid, tag, flags) );
    vec_t vec;  // empty vector
    // create a 0-lenght slot
    W_COERCE(page_p::insert_expand(0, 1, &vec) );
    return RCOK;
}
