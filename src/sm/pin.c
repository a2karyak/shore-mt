...
/*
 * $Id: pin.c,v 1.107 1996/07/19 23:00:01 nhall Exp $
 */
#define SM_SOURCE
#define PIN_C

#ifdef __GNUG__
//#pragma implementation "rsrc.h"
...
inline latch_mode_t lock_to_latch(lock_mode_t m)
{
    switch(m) {
    case SH:
    case UD:
                ...
                lpid_t pid, bool& pinned, bool cond, lock_mode_t lmode)
{
	...
	// loid index lookup and always pin the record
        W_COERCE( bf->fix(p, pid, page_p::t_file_p, LATCH_SH, FALSE) );
    }
			...
			int* old_value /* temporary: for degugging only */)
{
...
#else
	rc_t rc = lm->lock(_rid, lmode, t_long, 0, TRUE);
#endif
	if (rc) {
	    if (rc.err_num() == eMAYBLOCK || rc.err_num() == eLOCKTIMEOUT) {
	        ...
	        if (pinned()) unpin();
#ifndef MULTI_SERVER
	        W_DO_GOTO(err, lm->lock(_rid, lmode, t_long,
				WAIT_SPECIFIED_BY_XCT));
#else
	        W_DO_GOTO(err, lm->lock(_rid, lmode, t_long,
				WAIT_SPECIFIED_BY_XCT, FALSE, old_value));
#endif
	    } else {
	...
	// upgrade to an EX latch if all we had before was and SH latch
