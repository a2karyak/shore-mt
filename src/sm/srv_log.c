...
/*
 *  $Id: srv_log.c,v 1.16 1996/07/18 14:43:06 kupsch Exp $
 */
#define SM_SOURCE
#define LOG_C
#ifdef __GNUG__
...
)
{
	...
	smlevel_0::errlog->clog << error_prio 
	    << "Error: cannot open the log file(s) " << logdir <<flushl;
	return 0;
    }
...
#ifndef LOCAL_LOG
    off_t cc;
    if(me()->lseek(fd, _start + writebuf().firstbyte().lo(), SEEK_SET, cc)
	|| (uint4)cc != _start + writebuf().firstbyte().lo() ) 
#else
    if(::lseek(fd, _start + writebuf().firstbyte().lo(), SEEK_SET) 
	!= _start + writebuf().firstbyte().lo()) 
#endif
    {
	smlevel_0::errlog->clog << error_prio 
...
#ifndef LOCAL_LOG
    off_t cc;
    if(me()->lseek(fd, _start + lower, SEEK_SET, cc) 
	|| (uint4)cc != _start + lower )
#else
    if(::lseek(fd, _start + lower, SEEK_SET) != _start + lower) 
#endif
    {
	smlevel_0::errlog->clog << error_prio 
	    << "ERROR: could not seek to "
...
#ifndef LOCAL_LOG
	if (me()->read(fd, (const void *)(readbuf() + b), XFERSIZE) )
#else
	if (::read(fd, readbuf() + b, XFERSIZE) != XFERSIZE) 
#endif
	{
	    smlevel_0::errlog->clog << error_prio 
		    << "read() " << flushl;
