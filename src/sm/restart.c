...
/*
 *  $Id: restart.c,v 1.82 1996/06/25 14:52:20 nhall Exp $
 */
#define SM_SOURCE
#define RESTART_C
...
#ifdef __GNUG__
template class w_hash_t<dp_entry_t, lpid_t>;
template class w_hash_i<dp_entry_t, lpid_t>;
...
private:
    w_hash_t<dp_entry_t, lpid_t> tab; // hash table for dictionary
...
restart_m::recover(lsn_t master)
{
    ...
    log->SetInRecovery(true);

    ss_m::errlog->clog << info_prio << "Restart recovery:" << flushl;
#ifdef DEBUG
    {
     ...
     */
    ss_m::errlog->clog << info_prio << " analysis ..." << flushl;
     ...
     */
    ss_m::errlog->clog << info_prio << " redo ..." << flushl;
     ...
     *  pass in highest_lsn for debugging
     */
    ss_m::errlog->clog  << info_prio<< " undo ..." 
	<< " curr_lsn = " << curr_lsn
	<< flushl;

    undo_pass();

    ss_m::errlog->clog << info_prio << "Restart successful." << flushl;
    ss_m::errlog->clog << info_prio << "First new transaction will be " 
	<< xct_t::oldest_tid()
	<< flushl;
	...
	while ((xd = iter.next()))  {
	    ...
	    smlevel_0::coord_handle_t ch = xd->get_coordinator();
...
)
{
    ...
    /*
     *  Flag true after master checkpoint has been handled.
     *  Used to avoid processing an incomplete checkpoint.
     */
    bool chkpt_end_handled = false;
	...
	case logrec_t::t_xct_prepare_fi:
	    if (! chkpt_end_handled)  {
		// redo now, because our redo phase will
		// start after the master checkpoint
		r.redo(0);
	    }
	    break;
	...
	case logrec_t::t_chkpt_dev_tab:
		...
		for (uint i = 0; i < dv->count; i++)  {
		    ss_m::errlog->clog << info_prio << "  device " << dv->devrec[i].dev_name 
			 << " will be recovered as vid " << dv->devrec[i].vid
			 << flushl;
		    W_COERCE(io->mount(dv->devrec[i].dev_name, 
				       dv->devrec[i].vid));

		    w_assert3(io->is_mounted(dv->devrec[i].vid));
		}
	    }
	    break;
	...
	case logrec_t::t_mount_vol:
	     ...
	     */
	    w_assert3(chkpt_end_handled);  // mount & dismount shouldn't happen during a check point
	...
	case logrec_t::t_chkpt_end:
	    ...
	    // We should not encounter more than one complete chkpt
	    w_assert3(!chkpt_end_handled);

#endif /*DEBUG*/

	    chkpt_end_handled = true;
	    break;
...
#undef highest_lsn
{
	...
	if (!r.valid_header(lsn)) {
	    ...
	    << "Internal error during redo recovery." << endl;
	    ss_m::errlog->clog << error_prio 
	...
	if ( r.is_redo() ) {
		...
		if (r.tid() != null_tid)  {
		    ...
		    if(xd) {
		        // should not be a repeat of a
		        // checkpointed prepare tx
		        w_assert1(xd->state()==xct_t::xct_active);
	    ...
	    } else if(dptab.look_up(r.pid(), &rec_lsn) && lsn >= *rec_lsn)  {
		/*
		 ...
		 *   
		 */
#define TRUST_NEW_PAGE_LSN 1
#if TRUST_NEW_PAGE_LSN

		DBG(<< "TRUST_NEW_PAGE_LSN");
		W_COERCE( page.fix(r.pid(), 
		    page_p::t_any_p, LATCH_EX, 0, TRUE) );
...
{
    w_hash_i<dp_entry_t, lpid_t> iter(tab);
    /*
...
ostream& operator<<(ostream& o, const dirty_pages_tab_t& s)
{
    ...
    w_hash_i<dp_entry_t, lpid_t> iter(s.tab);
    const dp_entry_t* p;
    while ((p = iter.next()))  {
	o << " Page " << p->pid
	<< " lsn " << p->rec_lsn
