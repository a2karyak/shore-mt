...
/*
 *  $Id: smfile.c,v 1.27 1996/06/25 14:43:55 nhall Exp $
 */
#define SM_SOURCE
#define SMFILE_C
...
{
    /*
     *  lock store in EX and flush store pages cached
     */
    ...
    /*
     *  set the io store flag
     */
    uint4_t flags = _make_store_flag(property);
    sdesc_t* sd;
    W_DO( dir->access(stid, sd, NL) );

    W_DO( io->set_store_flags(stid, flags) );
    ...
    store_property_t&	property)
{
    ...
    if (flags & page_p::st_tmp)  {
	property = t_temporary;
    } else if (flags & page_p::st_regular) {
	property = t_regular;
    } else if (flags & page_p::st_no_log)  {
	property = t_no_log;
    } else {
	W_FATAL(eINTERNAL);
    }
		   ...
		   const serial_t& logical_id)
{
    ...
    uint4_t st_flag = _make_store_flag(property);
    ...
    stid_t lg_stid;
    W_DO( io->create_store(vid, 100/*unused*/, st_flag, lg_stid) );
    ...
    sinfo_s sinfo(fid.store, t_file, 100/*unused*/, 100/*unused*/, t_bad_ndx_t,
		      property, first.page, logical_id, 0, 0);
                   ...
                   const serial_t& logical_id)
{
    ...
    sinfo_s sinfo(fid.store, t_file, 100/*unused*/, 100/*unused*/,
		  t_bad_ndx_t, property, first.page, logical_id, 0, 0);
    sinfo.set_large_store(lg_stid.store);
    stpgid_t stpgid(fid);
    W_DO( dir->insert(stpgid, sinfo) );
...
ss_m::_destroy_file(const stid_t& fid)
{
    ...
    if (store_flags & page_p::st_tmp)  {
	// for temporary files, we can discard the buffers to save
	// some disk I/O.
...
ss_m::_destroy_n_swap_file(const stid_t& old_fid, const stid_t& new_fid)
{
    ...
    uint4_t old_store_flags, new_store_flags;
    ...
    if (old_store_flags & page_p::st_tmp)  {
	// for temporary files, we can discard the buffers to save
	// some disk I/O.
	W_IGNORE( bf->discard_store(old_fid) );
    }
    
    if (new_store_flags & page_p::st_tmp)  {
	// for temporary files, we can discard the buffers to save
	// some disk I/O.
