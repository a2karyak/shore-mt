...
/*
 *  $Id: unix_log.c,v 1.16 1996/03/27 17:58:35 bolo Exp $
 */
#define SM_SOURCE
#define LOG_C
#ifdef __GNUG__
    ...
    : srv_log(rdbufsize, wrbufsize, shmbase)
{
	...
	*/

	{
		...
		smlevel_0::errlog->clog << error_prio <<
		    "   can't write skip rec to log ..." << flushl;
		W_FATAL(eOS);
	    }
	    off_t o = pos;
	    o += s->length();
		...
		if ( fwrite(junk, o, 1, f) != 1)  {
		    smlevel_0::errlog->clog << error_prio <<
		    "   can't round out log block size ..." << flushl;
		    W_FATAL(eOS);
		}
		delete[] junk;
		o = 0;
	    ...
	    if( fsync(fileno(f)) < 0) {
		smlevel_0::errlog->clog << error_prio <<
		    "   can't sync fsync truncated log ..." << flushl;
		W_FATAL(eOS);
	    }
	    {
#ifdef DEBUG
...
#endif
	    {
		perror("unix_log(warning): fclose");
	    }
	}
	p->_clear();
    }
...
)
{
    ...
    fd  = ::open(fname, O_RDWR | O_SYNC | O_CREAT,  0744);
    if(fd < 0 ) {
	smlevel_0::errlog->clog << error_prio
    ...
    DBG(<<"OPEN " <<fname);
    if(me()->open(fname, O_RDWR, 0, fd))  {
	fd = -2;
    }
    DBG(<< " OPEN " << fname << " returned " << fd);
    if(fd < 0 ) {
	DBG(<<"");
	smlevel_0::errlog->clog << error_prio
	    << "ERROR: cannot open log file: " << fd  << flushl;
	...
	if(me()->ftruncate(fd,  XFERSIZE) )
#else
	if(::ftruncate(fd,  XFERSIZE) != 0)
#endif
	{
	     smlevel_0::errlog->clog << error_prio
...
)
{
	...
	DBG(<< " OPEN " << fname);
	fd = 0;
	if(me()->open(fname, O_RDONLY, 0, _fhdl_rd))  {
	    fd = -2;
	} 
#else
