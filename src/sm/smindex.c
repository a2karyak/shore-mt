...
/*
 *  $Id: smindex.c,v 1.66 1996/03/19 21:52:37 nhall Exp $
 */
#define SM_SOURCE
#define SMINDEX_C
#include "sm_int.h"
    ...
    const serial_t& 	serial)
{
	...
	<<stid<<" " <<serial);
    stpgid_t stpgid;
    W_DO(_create_index(vid, ntype, property, key_desc,
		       FALSE, stpgid, serial));
    stid = stpgid;
		   ...
		   serial_t& lstid)
{
    ...
    W_DO(_create_index(vid, ntype, property,
		       key_desc, use_1page_store, stpgid, lstid));
...
ss_m::find_assoc(const lvid_t& lvid, const serial_t& serial,
		 const vec_t& key, void* el, smsize_t& elen, bool& found)
{
    SM_PROLOGUE_RC(ss_m::find_assoc, in_xct, 0);
...
ss_m::find_md_assoc(const lvid_t& lvid, const serial_t& serial,
		 const nbox_t& key, void* el, smsize_t& elen, bool& found)
{
    ...
    const serial_t& 	logical_id)
{
	...
	W_DO(io->alloc_pages(stid_t(vid, small_store_id), 
			     lpid_t::null, 1, &root))
    ...
    const vec_t& 	el)
{
    ...
    // on the index
    concurrency_t	cc = t_cc_kvl;	// lock granularity needed
 ...
 *--------------------------------------------------------------*/
rc_t
ss_m::_destroy_all_assoc(const stpgid_t& stpgid, const vec_t& key, int& num)
{
    sdesc_t* sd;
    ...
    stid_t& 		new_stid)
{
	...
	W_DO( io->create_store(stpgid.vol(), sinfo.eff, 
			       _make_store_flag(sinfo.property), new_stid) );
