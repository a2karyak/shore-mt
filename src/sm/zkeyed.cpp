...
zkeyed_p::shift(slotid_t idx, zkeyed_p* rsib, bool compressed)
{
    ...
    for (int i = start_simple_move; i < n && (! rc); ) {
	int j;
...
)
{
	 ...
	 * splitting a btree page), let's try something quick first: 
	...
	W_DO( page_p::insert_expand(idx + 1, 1, &v, false, false) );
	return RCOK;
    }
