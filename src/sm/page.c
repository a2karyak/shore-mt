...
/*
 *  $Id: page.c,v 1.96 1996/05/03 04:07:45 kupsch Exp $
 */
#define SM_SOURCE
#define PAGE_C
#ifdef __GNUG__
...
page_s::space_t::usable(xct_t* xd)
{
    ...
    if (_rflag && xd)  {
        if (xd->status() == smlevel_1::xct_aborting)  {
	    /*
	       ...
	       uint4_t page_flags) 
{
    ...
    // w_assert3((store_flags() & st_no_log) == 0); // expect only logging on 
    /*
     *  Check alignments
     */
...
/*********************************************************************
 *
 *  page_p::fix(pid, ptag, mode, page_flags, ignore_store_id, refbit)
 *
 *
    ...
    int			refbit)
{
    ...
    if (_pp && _pp->pid == pid && _mode == m)  {
	/*
	 *  We have already fixed the page... do nothing.
	 */
    } else {
	if (_pp)  bf->unfix(_pp, false, _refbit);
	W_DO( bf->fix(_pp, pid, ptag, _mode = m, 
		      (page_flags & t_virgin) != 0, ignore_store_id) );
    }
    _refbit = refbit;
 ...
 *********************************************************************/
rc_t
page_p::reclaim(int idx, const cvec_t& vec)
{
    /*
     *  Only valid for pages that need space reservation
     */
 ...
 *********************************************************************/
rc_t
page_p::find_slot(uint4 space_needed, int& ret_idx, slotid_t start_search)
{
    /*
     *  Only valid for pages that need space reservation
     */
...
/*********************************************************************
 *
 *  page_p::insert_expand(idx, cnt, vec[])
 *
 ...
 *********************************************************************/
rc_t
page_p::insert_expand(int idx, int cnt, const cvec_t vec[])
{
     ...
     */
    W_DO( _pp->space.acquire(total, 0, xct()) );
 ...
 *********************************************************************/
rc_t
page_p::remove_compress(int idx, int cnt)
{
    w_assert1(! rsvd_mode() );
    w_assert1(idx >= 0 && idx < _pp->nslots);
...
/*********************************************************************
 *
 *  page_p::set_bit(int idx, bit)
 *
 *  Set a bit of slot idx.
 *
 *********************************************************************/
rc_t
page_p::set_bit(int idx, int bit)
{
    /*
 ...
 *********************************************************************/
rc_t
page_p::clr_bit(int idx, int bit)
{ 
    /*
     *  Compute the byte address
     */
 ...
 *********************************************************************/
rc_t
page_p::splice(int idx, int cnt, splice_info_t info[])
{
    for (int i = cnt; i >= 0; i--)  {
 ...
 *********************************************************************/
rc_t
page_p::splice(int idx, int start, int len, const cvec_t& vec)
{
    int vecsz = vec.size();
    w_assert1(idx >= 0 && idx < _pp->nslots);
 ...
 */
page_s::splice(int idx, int cnt, splice_info_t info[])
{
...
page_p::page_usage(int& data_size, int& header_size, int& unused,
		   int& alignment, page_p::tag_t& t, int& no_slots)
{
