...
/*
 * $Id: rdtree.c,v 1.32 1996/04/09 20:44:38 nhall Exp $
 */
#define SM_SOURCE
#define RDTREE_C
...
rsetob_cmp(const rangeset_t& key, const rangeset_t& rset, sob_cmp_t type)
{
	...
	case t_overlap: 	// overlap match
	    return (rset || key);
	case t_cover: 		// coverage: key covers rset
	    return (key / rset);
	case t_inside:		// containment: rset contained in key
	    return (rset / key);
	default:
	    w_assert1(0);
	    return FALSE;
			  ...
			  oper_t oper) 
{
    ...
    for (found = FALSE, pid.page = rootpage.empties();
	 ...
	 found = page.search(key, t_exact, smap, tmp, &el);
	 count = page.nrecs();
	 if (found) 
...
rdtree_p::insert(const rangeset_t& key, const cvec_t& el, shpid_t child)
{
    ...
    if ( search(key, t_exact, smap, num_slot, &el) ) {
		    ...
		    rtstk_t& pl, oper_t oper)
{
	...
	found = page.search(key, t_inside, smap, num_slot);
    ...
    found = page.search(key, t_exact, smap, num_slot, &el);
    count = page.nrecs();
