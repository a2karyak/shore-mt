...
/*
 *  $Id: rtree.c,v 1.101 1996/06/10 21:20:39 nhall Exp $
 */
#define SM_SOURCE
#define RTREE_C
...
rc_t
rtree_p::format(const lpid_t& pid, tag_t tag, uint4_t flags)
{
...
//
rc_t
rtree_base_p::format(const lpid_t& pid, tag_t tag, uint4_t flags)
{
    rtctrl_t rtc;
    rtc.level = 1;
    rtc.dim = 2;
...
//
void 
rtree_p::pick_optimum(const nbox_t& key, int2& ret_slot)
{
    w_assert3( is_node() );
...
rtree_p::insert(const nbox_t& key, const cvec_t& el, shpid_t child)
{
    ...
    if ( search(key, t_exact, smap, num_slot, &el, child) ) {
...
//
static int
sob_cmp(const nbox_t& key, const nbox_t& box, sob_cmp_t type)
{
    switch(type) {
	case t_overlap: 	// overlap match
	    return (box || key);
	case t_cover: 		// coverage: key covers box
	    return (key / box);
	case t_inside:		// containment: box contained in key
	    return (box / key);
	default:
	    w_assert1(0);
	    return false;
...
//
bool 
rtree_p::spatial_srch(const nbox_t& key, sob_cmp_t ctype, 
...
//
bool 
rtree_p::query(const nbox_t& key, sob_cmp_t ctype, u_char smap[],
			int2& num_slot)
{
    w_assert3(!is_leaf());
    sob_cmp_t cond = (ctype==t_exact)? t_inside : t_overlap;
...
//
bool 
rtree_p::search(const nbox_t& key, sob_cmp_t ctype, u_char smap[],
			int2& num_slot, const cvec_t* el, const shpid_t child)
{
    switch(ctype) {
	case t_exact: 		// exact match search: binary search
	    num_slot = 1;
	    if (!el) {
		cvec_t dummy;
	...
	case t_overlap: 	// overlap match
	case t_cover: 		// coverage: any rct cover the key
	case t_inside:		// containment: any rct contained in the key
    ...
    oper_t 		oper)
{
	...
	found = ((rtree_p)pl.top().page).search(key, t_inside, smap, num_slot);
    ...
    found = ((rtree_p)pl.top().page).search(key, t_exact, smap, num_slot, &el);
    ...
    const cvec_t&	elem)
{
	...
	for (int i=0; i<rtstk_t::max_rtstk_sz; i++) lvl_split[i] = false;
        if (leaf.pid()!=root) {
	    X_DO ( _ov_treat(root, pl, key, leaf, lvl_split) );
        } else {
    	    X_DO( _split_page(root, pl, key, leaf, lvl_split) );
	    split = true;
	}
    ...
    rtree_stats_t& stats)	// O- index stats
{
    ...
    X_DO( fi->first_page(src, pid) );
    for (bool eof = false; ! eof; ) {
	X_DO( page[i].fix(pid, LATCH_SH) );
	for (int s = page[i].next_slot(0); s; s = page[i].next_slot(s)) {
	    const record_t* r;
	    ...
	    X_DO( ld_stack.add_to_stk(key, el, 0, 0) );
	    pr = r;
        }
	i = 1 - i;
	X_DO( fi->next_page(pid, eof) );
    }

    X_DO( ld_stack.wrap_up() );
