...
/*
 *  $Id: lid.c,v 1.127 1996/07/09 20:41:20 nhall Exp $
 */
#define SM_SOURCE
#define LID_C
		  ...
		  const lpid_t& lid_index, const lpid_t& remote_index)
{
	...
	w_assert3(found && klen == sizeof(serial_t) && max_local_id.is_local());
	...
	// prev key should have been found
	w_assert3(found && klen == sizeof(serial_t));
	w_assert3(found && klen == sizeof(serial_t) && max_remote_id.is_remote());
	//cout << "Max Remote ID = " << max_remote_id << endl;

#ifdef OLDCODE
	/*
	 * Find the largest local and remote serial numbers
	 * Since btree keys are just strings (for now at least)
	 * this search must be done differently for little endian
	 * machines since the byte order does not allow string
	 * compares to work properly.
	 */

	serial_t check_max_local_id(0, FALSE);
	serial_t check_max_remote_id(0, TRUE);
	{
	    /*
	     * The following code is used to find the largest local
	     * and remote serial numbers on the volume.  This
	     * code is a TEMPORARY HACK until indexes with typed keys
	     * available.  Once this is available, the code should be
	     * written to do a binary search on the btree looking
	     * for the largest keys.
	     */
	    serial_t curr_id;
	    int	     len_max = sizeof(serial_t);
	    bt_cursor_t	cursor;
	    W_DO( bt->fetch_init(cursor, lid_index, 
				 lid_key_type_len, lid_key_type,
				 TRUE /*unique*/, t_cc_none,
	    ...
	    rc_t rc;
	    if (rc) {;}
	    while ( (!(rc = bt->fetch(cursor))) &&
		    cursor.key())  {
		w_assert1(len_max <= cursor.klen());
		w_assert1(cursor.klen() == sizeof(serial_t));
		memcpy(&curr_id, cursor.key(), cursor.klen());
		if (curr_id.is_remote()) {
		    if (curr_id > check_max_remote_id) check_max_remote_id = curr_id;
		} else {
		    if (curr_id > check_max_local_id) check_max_local_id = curr_id;
		}
	    }
	    if (rc) {
	...
	w_assert3(max_local_id == check_max_local_id);
	w_assert3(max_remote_id == check_max_remote_id);
#endif /* OLDCODE */
		...
		vid_t& vid, bool& found, bool& cache_hit)
{
	...
	if (lid_index.vol().is_remote()) {
	    DBG(<<"found lid_index.vol= " << lid_index.vol() <<" lid=" << lid);
	    W_DO(rm->lookup_lid(lid_index.vol(), lid, entry));
...
lid_m::vol_lid_info_t* lid_m::_lookup_lvid(const lvid_t& lvid, vol_restrict_t vol_restrict)
{
	...
	if(comm_m::use_comm()) {
	    // the volume was not found, so it may be remote.
	    // Have the remote manager try to mount the volume
	    DBG(<< "volume " << lvid << "not mounted");

	    _vol_mutex.release(); // so mount can proceed
	    // Since we release the mutex, there is a slight
	    // chance the mount can succeed but the volume be
	    // unmounted before the mutex is reacquired.
	    while (!rm->mount_volume(lvid)) {
		W_COERCE( _vol_mutex.acquire() ); // so mount can proceed
		vol_info = _vol_table->lookup(lvid);
		if (vol_info != NULL) break;
		_vol_mutex.release(); // so mount can proceed
	    }
	    if (!_vol_mutex.is_mine()) W_COERCE( _vol_mutex.acquire() );
	} else {
	    return NULL;
	}
    }
