...
/*
 *  $Id: file.c,v 1.145 1996/07/01 22:07:19 nhall Exp $
 */
#define SM_SOURCE
...
file_m::first_page(const stid_t& fid, lpid_t& pid, bool* allocated,
						bool lock, pid_cache_t* cache)
{
    if (fid.vol.is_remote()) {
#ifdef MULTI_SERVER
	if (allocated) W_FATAL(eNOTIMPLEMENTED);
	pid._stid = fid;

	if (! cache) {
	    int num = 1;
	    W_DO(rm->get_pids(pid.stid(), lock, FALSE, 0, num, &pid.page));
	    w_assert3(num == 1);
	} else {
	    cache->reset(0);
	    int num = cache->size();
	    W_DO(rm->get_pids(pid.stid(), lock, FALSE, 0, num, cache->pids()));
	    cache->reset(num);
	    w_assert3(num > 0);
	    pid.page = cache->next_page(pid);
	}
#else
	W_FATAL(eBADSTID);
...
rc_t
file_m::last_page(const stid_t& fid, lpid_t& pid, 
	bool* allocated,
	bool lock, pid_cache_t* cache)
{
    if (fid.vol.is_remote()) {
				...
				bool lock, pid_cache_t* cache)
{
	...
	if (! cache) {
	    int num = 1;
	    W_DO(rm->get_pids(pid.stid(), lock, FALSE, pid.page, num,
								&pid.page));
	    if (num != 1) {
		w_assert3(num == 0);
		eof = TRUE;
	    ...
	    shpid_t spid = 0;
	    spid = cache->next_page(pid);
	    if (spid) {
		pid.page = spid;
	    } else {
		spid = cache->last_page();
		int num = cache->size();
		W_DO(rm->get_pids(pid.stid(), lock, FALSE, spid, num,
								cache->pids()));
		cache->reset(num);
		if (num == 0)	eof = TRUE;
		else		pid.page = cache->next_page(pid);
	    }
	}
#else
	W_FATAL(eBADPID);
...
{
    /*
     *  Allocate a new page and compensate.  The page allocation
     *	and formatting must be compensated to ensure that the
     *	page remains in the file even if the transaction is aborted.
     *	This is necessary since other transactions may be adding
     *	records to the page.
     *
     *  There would be no need to compensate the page allocation if
     *  the file or page were EX locked and only 1 thread-per-xct.  
     *  In that case this is the only
     *  transaction using the page and therefore it is safe to 
     *  remove it (undo the allocation) on abort.
     */
    lsn_t anchor = xct()->anchor();

    if(last_page) {
	X_DO( io->alloc_new_last_page(fid, near, allocPid) );
	w_assert3(lmode == EX);
    } else {
	X_DO( io->alloc_pages(fid, near, 1, &allocPid, forward_alloc) );
    }
    
    if(lmode != EX && lmode != NL) {
	X_DO(lm->lock(allocPid, lmode, t_long, WAIT_SPECIFIED_BY_XCT));
    }

    X_DO( page.fix(allocPid, LATCH_EX, page.t_virgin) );
...
file_m::_append_large(file_p& page, int2 slot, const vec_t& data)
{
    ...
    const int	max_pages = 16;	// max pages to alloc per request
    ...
    // TODO: boost max_pages in _append_large to 64
    DBG( << "boost max_pages in _append_large to 64" );
			       ...
			       const vec_t& data, smsize_t left_to_append)
{
	...
	lgdata_p lgdata;
	W_DO( lgdata.fix(new_pages[i], LATCH_EX, lgdata.t_virgin) );
...
rc_t
file_p::fix_record(const rid_t& rid, latch_mode_t m, bool ignore_store_id)
{
    w_assert3(rid.pid.is_remote());
    w_assert3(!_pp || bf->is_bf_page(_pp));
   
    if (_pp && _pp->pid == rid.pid && _mode == m)  {
        /*
         *  We have already fixed the page... do nothing.
         */
    } else {
        if (_pp)  bf->unfix(_pp, false, _refbit);
        W_DO(bf->fix_record(_pp, rid, _mode = m));
	ntoh();
    }
    _refbit = 1;
...
#else
	if (rc = llm->lock(rid, EX, t_long, WAIT_IMMEDIATE))  {
#endif
	    /*
...
rc_t
file_p::get_rec(int idx, record_t*& rec)
{
    rec = 0;
    if (! is_rec_valid(idx)) 
	return RC(eBADSLOTNUMBER);
...
rc_t
file_p::splice_data(int idx, smsize_t start, smsize_t len, const vec_t& data)
{
    record_t*   rec;
...
rc_t
file_p::append_rec(int idx, const vec_t& data)
{
    record_t*   rec;
    W_COERCE( get_rec(idx, rec) );
    w_assert3(rec != NULL);
...
rc_t
file_p::truncate_rec(int idx, uint4 amount)
{
    record_t*   rec;
    W_COERCE( get_rec(idx, rec) );
    w_assert3(rec);
...
rc_t
file_p::set_rec_len(int idx, uint4 new_len)
{
    record_t*   rec;
    W_COERCE( get_rec(idx, rec) );
    w_assert3(rec != NULL);
...
rc_t
file_p::set_rec_flags(int idx, uint2 new_flags)
{
    record_t*   rec;
    W_COERCE( get_rec(idx, rec) );
    w_assert3(rec);
