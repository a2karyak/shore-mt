...
/*
 *  $Id: scan.c,v 1.106 1996/07/09 20:41:24 nhall Exp $
 */
...
{
    lid_m* const lid = ss_m::lid;
    dir_m* const dir = ss_m::dir;
...
/*********************************************************************
 *
 *  scan_index_i::scan_index_i(stid, c1, bound1, c2, bound2, cc)
 *
...
scan_index_i::scan_index_i(
    ...
    : xct_dependent_t(xct()), stpgid(stid_), ntype(ss_m::t_bad_ndx_t),
      cond2(c2), serial(serial_t::null), _eof(FALSE),
...
scan_index_i::scan_index_i(
    ...
    const cvec_t& 	bound2_, 
    concurrency_t 	cc) 
    : xct_dependent_t(xct()), ntype(ss_m::t_bad_ndx_t), cond2(c2),
    ...
    const cvec_t& 	b2)
{
     ...
     *   == 30 <= 20
     */
    if (! (cond == eq || cond == gt || cond == ge) )  {
	_error_occurred = RC(eBADCMPOP);
	return;
    }

    if (! (c2 == eq || c2 == lt || c2 == le) )  {
	_error_occurred = RC(eBADCMPOP);
	return;
    }
    if (cond == eq || c2== eq) {
    ...
    case t_uni_btree:
	{
	    ...
	    cvec_t* elem = &(inclusive ? cvec_t::neg_inf : cvec_t::pos_inf);
...
scan_rt_i::scan_rt_i(const stid_t& stid_, sob_cmp_t c, const nbox_t& qbox, concurrency_t cc) 
...
scan_rt_i::scan_rt_i(const lvid_t& lvid, const serial_t& stid_,
		     sob_cmp_t c, const nbox_t& qbox, concurrency_t cc) 
...
void scan_rt_i::_init(sob_cmp_t c, const nbox_t& qbox)
{
    _finished = false;
...
scan_rdt_i::scan_rdt_i(const lvid_t& lvid, const serial_t& stid_,
		     sob_cmp_t c, const rangeset_t& qset, concurrency_t cc) 
...
void scan_rdt_i::_init(sob_cmp_t c, const rangeset_t& qset)
{
    _finished = false;
...
: xct_dependent_t(xct()), stid(stid_), curr_rid(start), _eof(FALSE),
  _error_occurred(), _lfid(serial_t::null), _cc(cc), _srv_pglock(FALSE), _pids(0)
{
...
: xct_dependent_t(xct()), stid(stid_), _eof(FALSE), _error_occurred(),
  _lfid(serial_t::null), _cc(cc), _srv_pglock(FALSE), _pids(0)
{
    INIT_SCAN_PROLOGUE_RC(scan_file_i::scan_file_i, 0);
    if (_init()) return;
}
...
scan_file_i::scan_file_i(const lvid_t& lvid, const serial_t& lfid,
				const serial_t& start_rid, concurrency_t cc) 
: xct_dependent_t(xct()), _eof(FALSE), _error_occurred(),
  _lfid(serial_t::null), _cc(cc), _srv_pglock(FALSE), _pids(0)
{
    INIT_SCAN_PROLOGUE_RC(scan_file_i::scan_file_i, 0);
...
scan_file_i::_init_logical(const lvid_t& lvid, const serial_t& lfid) 
{
    ...
    _error_occurred = validate_stid(id, stid);
    if (_error_occurred) return _error_occurred;
...
scan_file_i::_init(
    bool for_append) 
{
    if (stid.vol.is_remote()) {
        _pids = new pid_cache_t(for_append? 2: 1000, stid);
    ...
    if (mode != SH) {
        lock_mode_t fmode;
        W_DO(lm->query(stid, fmode, xct()->tid(), TRUE));
        w_assert3(fmode != NL);
        if (fmode == IS || fmode == IX) _srv_pglock = TRUE;
    }
#endif
	...
	_error_occurred = for_append?
	    fi->last_page(stid, curr_rid.pid, 0, _srv_pglock, _pids): 
	    fi->first_page(stid, curr_rid.pid, 0, _srv_pglock, _pids);
    ...
    _error_occurred = fi->next_page(_next_pid, eof, 0, _srv_pglock, _pids);
    if(for_append) {
	w_assert3(eof);
...
scan_file_i::_next(pin_i*& pin_ptr, smsize_t start, bool& eof)
{
...
#else
		if (_error_occurred = llm->lock(curr_rid, _rec_lock_mode, t_long, WAIT_IMMEDIATE))  {
#endif
	...
	if (curr_rid.slot == 0) {
		...
		// remember the next pid
		bool tmp_eof;
		_error_occurred = fi->next_page(_next_pid, tmp_eof, 0,
							_srv_pglock, _pids);
		if (_error_occurred)  {
		    return _error_occurred;
		}
...
append_file_i::append_file_i(const stid_t& stid_) 
 : scan_file_i(stid_, t_cc_file)
{
    INIT_SCAN_PROLOGUE_RC(append_file_i::append_file_i, 0);
    W_IGNORE(_init(true));
    if(_error_occurred) return;
    if(_error_occurred = lm->lock(_next_pid.stid(), EX, 
...
append_file_i::append_file_i(const lvid_t& lvid, const serial_t& lfid)
 : scan_file_i(lvid, lfid, t_cc_file)
{
    INIT_SCAN_PROLOGUE_RC(append_file_i::append_file_i, 0);
    if (_init_logical(lvid, lfid)) return;
    _init(true);
	...
	)
{
	...
	hdr, len_hint, data, serial, *sd, _page(), curr_rid) );
