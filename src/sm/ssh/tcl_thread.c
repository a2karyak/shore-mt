...
/*
 *  $Id: tcl_thread.c,v 1.69 1996/06/10 20:12:05 kupsch Exp $
 */
#define TCL_THREAD_C
...
extern test_dispatch(ClientData, Tcl_Interp* ip, int ac, char* av[]);
#endif
...
#if defined(MULTI_SERVER) && defined(MARKOS_TEST)
    Tcl_CreateCommand(ip, "test", test_dispatch, 0, 0);
#endif
...
tcl_client_thread_t::tcl_client_thread_t(Node* node, Port* port,
			   Tcl_Interp* pip)
: tcl_thread_t(0, 0, pip, true), _node(node), _port(port) 
{
	W_COERCE(unblock());
}

void tcl_client_thread_t::run()
{
    FUNC(tcl_client_thread_t::run);

    Buffer send_buf(sizeof(rctrl_msg_t));
    Buffer rec_buf(sizeof(rctrl_msg_t));
	...
	Tcl_DStringInit(&cmd);
	rc = rctrl_msg_t::receive_tcl_string(cmd, *_port, rec_buf, from_node);
	if (rc) {
	    if (rc.err_num() == scommDead) {
		assert(from_node == _node);
	...
	result = Tcl_Eval(ip, Tcl_DStringValue(&cmd));
	Tcl_DStringFree(&cmd);

	// send result to client
	send_msg->set_type(rctrl_msg_t::command_msg);
	send_msg->content.cmd.error_code = result;
	rc = rctrl_msg_t::send_tcl_string(ip->result, *_node, rec_msg->reply_port(), send_buf);
	if (rc) {
	    cerr << "error in sending reply message" << endl;
	    W_COERCE(rc);
	}
	DBG( << "sent reply message " << ip->result );
    }

    cerr << "DO PROPER CLEANUP in tcl_client_thread_t::do_work" << endl;
    // such as free-ing _port
}

tcl_client_thread_t::~tcl_client_thread_t()
{
    cerr << "tcl_client_thread_t DESTRUCTOR was called" << endl;
}
