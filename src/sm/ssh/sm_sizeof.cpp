/*<std-header orig-src='shore'>
 ...
 $Id: sm_sizeof.cpp,v 1.6 2004/01/08 05:09:17 bolo Exp $
...
struct ssh_type_info_t {
	const char	*type;
	size_t		size;
};

static const ssh_type_info_t	type_info[] = {
	TYPE_INFO(rid_t),
	TYPE_INFO(lrid_t),
	TYPE_INFO(stid_t),
	TYPE_INFO(sdesc_t),
...
#if 0 && !defined(FORCE_EGCS) && !defined(GCC_BROKEN_WARNINGS) && !defined(GCC_VER_3_WARNINGS)
...
int	t_sm_sizeof(Tcl_Interp *, int argc, TCL_AV char **argv)
{
	...
	if (argc == 1) {
		for (unsigned j = 0; j < numberof(type_info); j++) {
			const ssh_type_info_t &t = type_info[j];
