/*<std-header orig-src='shore'>
 ...
 $Id: shell.cpp,v 1.330 2007/05/18 21:50:58 nhall Exp $
...
#ifdef notyet
	/* This works fine, general case, but is it overly generous? */
	char *t;
	long l = strtol(s, &t, 0);
	if (s != t && *t == 0) {
		result = l;
...
#ifdef notyet
	/* Probably best to output all pointers as a fixed representation
	   instead of relying upon a platform's specific behavior -- because
	   then each platform must be accounted for */
	return o << (ptrdiff_t) ptr;
#else
#ifdef _MSC_VER
#if _MSC_VER >= W_MSC_VER(7,0)
	/* XXX 7.0 doesn't append a 0x prefix to hex addresses, and
	   the resulting %08x number can be interpreted as decimal,
	   octal, or hexadecimal depending upon the digits it uses.*/
	return o << "0x" << ptr;
#else
	return o << ptr;
#endif
...
/* XXX Tcl_Interp is a notyet to generate error messages correctly. */
...
#ifdef notyet
	// see notes above on generic guaranteed-to-work represenation
	// xxx although hex would be best. to keep values/ptrs seperate
	l = strtol(s, &t, 10);
#else
#if defined(_MSC_VER)
#if _MSC_VER >= W_MSC_VER(7,0)
	// force this to prevent octal or decimal
	// interpretation by accident
	l = strtol(s, &t, 16);
#else
	l = strtol(s, &t, 0);
#endif
...
t_preemption_simulated(Tcl_Interp* ip, int ac, TCL_AV char* [])
{
    ...
    Tcl_AppendResult(ip, tcl_form_flag(preemption_simulated()), 0);
    return TCL_OK;

#else /* USE_SSMTEST */

    Tcl_AppendResult(ip, tcl_form_flag(0), 0);
    return TCL_OK;
#endif /* USE_SSMTEST */
}
...
t_simulate_preemption(Tcl_Interp* ip, int ac, TCL_AV char* av[])
{
	...
	bool	flag = false;
	if (tcl_scan_flag(av[1], flag) != TCL_OK) {
		Tcl_AppendResult(ip, "usage: simulate_preemption on|off", 0);
		return TCL_ERROR;
	}

#ifdef USE_SSMTEST
	simulate_preemption(flag);
#else
	cerr << "Warning: simulate_preemption (USE_SSMTEST) not configured"
	     << endl;
...
t_restart(Tcl_Interp* ip, int ac, TCL_AV char* av[])
{
	...
	DO(stop_comm());  // stop smsh connection listen thread
#ifdef USE_COORD
...
t_set_escalation_thresholds(Tcl_Interp* ip, int ac, TCL_AV char* av[])
{
    ...
    if (listArgc != 3)  {
	Tcl_SetResult(ip, (char *) errString, TCL_VOLATILE);
...
t_co_sendtcl(Tcl_Interp* ip, int ac, TCL_AV char* av[])
{
	...
	w_reset_strstream(tclout); 
	tclout << smsh_err_name(error) << ends;
	Tcl_AppendResult(ip, tclout.c_str(), 0);
	w_reset_strstream(tclout);
	return TCL_ERROR;
    } else {
...
t_local_add(Tcl_Interp* ip, int , TCL_AV char* av[])
{
    ...
    {
	w_istrstream anon2(av[2]); anon2 >> t;
    }
    DO( co->add(g, t));
    return TCL_OK;
}
