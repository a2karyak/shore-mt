/*<std-header orig-src='shore'>
 ...
 $Id: sm_coord.cpp,v 1.66 2007/05/18 21:50:58 nhall Exp $
...
#ifdef W_TRACE
    DBGTHRD(<<"self() :");
    if(_w_debug.flag_on("SELF",__FILE__)) {
	_w_debug.clog << ": SELF IS ... " 
		<< self() << flushl;
    }
#endif
    if(myname) {
...
sm_coordinator::init_either(const char *myname, Endpoint*& named)
{
	 ...
	 * fix that to support a smsh that's got both.
	 */
...
) 
{
...
#ifdef W_TRACE
	if(_w_debug.flag_on("get_tcl",__FILE__)) {
	    _w_debug.clog 
		    << stamp
		    << ": RECVG ON "
		    << recvr
	...
	// our smsh protocol to be dependent on the SM 2pc protocol.
