/*<std-header orig-src='shore'>
 ...
 $Id: sm_mappings.cpp,v 1.39 2007/05/18 21:50:59 nhall Exp $
...
ep_map::add(int num, const char **list)
{
	...
	s = c;
	    ...
	    w_ostrstream ss;
	    ss << s << ends;
            w_rc_t e = _ns->lookup(ss.c_str(), ep);
	    W_DO(e);
#ifdef W_TRACE
	    _w_debug.clog << _slist_size << ": NS: " 
		    << s << " <-->  " <<  ep << flushl;
#endif 
	    w_assert3(ep);
	    w_assert3(ep.is_valid());
...
ep_map::refresh(const char *c, Endpoint& given, bool force_lookup)
{
   ...
   DBG(<<"refresh: given= " << given);
    ...
    w_ostrstream	ss;
    ss << s << ends;
    w_rc_t e = _ns->lookup(ss.c_str(), given);
    W_DO(e);
