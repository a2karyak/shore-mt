...
/*
 *   $RCSfile: unix_stats.C,v $
 *   $Revision: 1.4 $
 *   $Date: 1996/04/09 20:34:01 $
 */ 
...
ostream& operator<<( ostream& o, const unix_stats &s) 
{
	...
	usr=s.usertime();
	sys=s.systime();
#define FORM(u) form("%2.2f", ((((float)u)/s.iterations)/MILLION)) << #u <<" ";
	if(clk+usr+sys != 0) {
		o << "usec/iter for " << s.iterations << " iterations" << endl;
		// print in order that time(csh(1)) does:
		if(usr!= 0) { o << FORM(usr); }
		if(sys != 0) { o << FORM(sys); }
		if(clk!= 0) { o << FORM(clk); }
		if(clk!= 0) {
		    o << form("%2.2f %%",(float)((float)(usr+sys)/clk)*100) << " ";
		}
		o << endl;
	}
	if( (i=s.page_faults()) != 0) {
		o << form("%6d maj ",i);
	}
	if( (i=s.page_reclaims()) != 0) {
		o << form("%6d min ",i);
	}
	if( (i=s.swaps()) != 0) {
		o << form("%6d swp ",i);
