...
/*
 *  $Header: /p/shore/shore_cvs/src/fc/w_statistics.c,v 1.16 1996/05/03 03:17:32 kupsch Exp $
 */
		...
		for(i=0; i< m->count; i++) {
			...
			case 'v':
				if((s._print_flags & w_statistics_t::print_nonzero_only)==0 || 
					(m->values[i]._u.v != 0)) {
					out << ::form("%*.*s", field, field, m->strings[i]) << ": ";
					out << ::form("%10d",m->values[i]._u.v);
					out << endl;
				}
				break;
			case 'l':
			...
			case 'f':
				if((s._print_flags & w_statistics_t::print_nonzero_only)==0 || 
					(m->values[i]._u.f != 0.0)) {
					out << ::form("%*.*s", field, field, m->strings[i]) << ": ";
					out << ::form("%#10.6g",m->values[i]._u.f);
					out << endl;
				}
				break;
			default:
