...
// is handled the old way vs the new way in ssh 
bool newsort = true ;
...
) 
{
    ...
    metadata*		meta = &_meta[int(cookie)];

    return hilbert(rid, obj_in, meta, internal, key); 
}
...
make_random_box(char *object, w_base_t::uint4_t length)
{
    ...
    for(int i=0; i<4; i++) {
	int x = w_base_t::int4_t(random_generator::generator.lrand());
	if(x < 0) x = 0 - x;
...
make_random_alpha(char *object, w_base_t::uint4_t length)
{
	...
	while(j < lower_alpha || j > upper_alpha) {
	    n = w_base_t::uint1_t(random_generator::generator.lrand());
	    j = n & 0x7f;
	}
...
{
public:
   void freefunc(const void *p, smsize_t s) {
	nbox_t *b = (nbox_t *)p;
...
{
    generic_CSKF_cookie&	K = *(generic_CSKF_cookie*)cookie;
...
{
    generic_CSKF_cookie *g = (generic_CSKF_cookie *) lfunc_cookie;
