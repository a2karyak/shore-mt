...
#define STRINGMAX ss_m::page_sz
	...
	for(k=0; k<nkeys; k++) {
		...
		cout << "can't check large objects yet "<<endl;
		delete scan;
		return 1;
	    }
...
t_multikey_sort_file(Tcl_Interp* ip, int ac, char* av[])
{
    ...
    random_generator::generator.srand(99); // pick any seed
...
"vid nrecs runsize up|down uniq|dupok  file|index keep|nokeep deep|shallow carry|nocarry  nkeys type FVDNA [type FVDNA]*";
	...
	for(j=0; j < nrecs; j++) {
		...
		if(make_aligned[k]) {
			...
			buff = smsize_t(random_generator::generator.lrand() & 0x8);
		    }
		} // else buff = 0;
		DBG(<<"buff size " << buff);
		offset += buff;
...

		{
		    ...
		    // cerr << "test_bv using length " << length <<endl;
			    ...
			    length = 
			    smsize_t(random_generator::generator.lrand() 
			...
			// cout << "I/U 1";
			w_base_t::uint1_t i=
			w_base_t::uint1_t(random_generator::generator.lrand() & 0xff);
			length = 1;
			...
			// cout << "I/U 2";
			w_base_t::uint2_t i=
			w_base_t::uint2_t(random_generator::generator.lrand() & 0xffff);
			length = 2;
			memcpy(object, &i, length);
			}
			dlength = length;
			...
			// cout << "I/U 4";
			w_base_t::uint4_t i=
				random_generator::generator.lrand();
			length = 4;
			memcpy(object, &i, length);
			}
			dlength = length;
		    ...
		    case test_i8: {
			w_base_t::uint8_t i=
				random_generator::generator.qrand();
			length = 8;
			memcpy(object, &i, length);
			}
			dlength = length;
		    ...
		    case test_f4: {
			// cout << "F 4";
			float i = float(random_generator::generator.drand());
			length = 4;
			memcpy(object, &i, length);
			dlength = length;
			}
		    ...
		    case test_f8:{
			// cout << "F 8" ;
			double i = double(random_generator::generator.drand());
			length = 8;
			memcpy(object, &i, length);
			dlength = length;
			}
		...
		if(make_nullable[k] && (count_bv_objects <= 4)
		    ...
		    // the length info.
		    unsigned int xxx = int(random_generator::generator.lrand());
		    if(xxx & 0x00600200) {
			DBG(<<"forcing NULL");
			nulls++;
			meta[k].length = 0;
    ...
    if(kl.set_object_marshal(copyMOF, sort_keys_t::noUMOF, 0)) 
    {
	 ...
	 */
	if(internal.summary_thread.sort_duplicates > 0) {
	    /* 
		...
		cerr << "duplicates removed (" 
			<< internal.summary_thread.sort_duplicates 
	...
	if(int(internal.summary_thread.sort_recs_created) != nrecs) {
	    if(!uniq) {
		cerr << "recs_created = " 
			<< internal.summary_thread.sort_recs_created 
			<< "; expected " << nrecs << endl;
		w_assert1(0);
	    }
	}
