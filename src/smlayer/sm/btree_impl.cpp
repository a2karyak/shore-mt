...
tree_latch::unfix() 
{
     ...
     * to be unfixed.
     */
    w_assert3(_page.latch_mode() == LATCH_NL );
}
...
)
{
    ...
    W_DO(smlevel_0::io->alloc_pages(
	root.stid(), 
	near_p, 	// hint
	1, &pid, 	// n pages, array for output pids
	true, 	//  may_realloc 
	NL,	//  ignored
	true	// search file
	)) ;
...
pagain:
	...
	w_assert3(! p[0].is_fixed());
	w_assert3(! p[1].is_fixed());
	c = 0;
...
no_change:
	...
	if(p[1-c].is_fixed()) {
	    ...
	    DBGTHRD(<<"1-page btree: latch mode= " << int(leaf.latch_mode()));
